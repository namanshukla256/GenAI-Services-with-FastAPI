...
def extract_urls(text: str) -> list[str]:
    ...
    urls = re.findall(url_pattern, text) # 2 # url_pattern is the regex pattern to find URLs, text is the input text
    return urls
...
def parse_inner_html(html_string: str) -> list[str]:
    ...
    if content := soup.find_all("div", id="bodyContent"): # 3
...
async def fetch(session: aiohttp.ClientSession, url: str) -> str:
    ...
    async with session.get(url) as response: # 4 # Make an asynchronous GET request to the URL
...
async def fetch_all(urls: list[str]) -> str:
    ...
    async with aiohttp.ClientSession() as session: # 5 # Create an asynchronous HTTP session
